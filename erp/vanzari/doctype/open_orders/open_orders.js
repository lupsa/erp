...
//	},
	...
	before_submit(frm) {
		...
		for(let i = 0; i < produse_comandate.length; i++) {
			...
			var operatii = [];
			if(row.lista_operatii_cda){
				frappe.call({
					method: "frappe.client.get",
					args: {
						doctype: "Lista Operatii",
						name: row.lista_operatii_cda,
					},
					async:false,
					callback(r) {
						r.message.lista_operatii.map((el, i) => {
							operatii[i] = {
								"operatia": el.operatia,
								"numar_ore_manopera": el.ore_manopera,
								"obvervatii": el.observatii
							}
						});
					}
				});
